...
public class HotelP implements DModel {
   ...
   private  TemporalFormulaExtension temporalFormula;
    ...
    public Formula tempDecls(){
        Formula x22 = lastkey.in(room.product(key)).always();/*TEMPORAL OP*/ // lastkey in Room -> Key -> Time
        Formula x27 = occupant.in(room.product(guest)).always();/*TEMPORAL OP*/ // occupant in Room -> Guest -> Time
        Formula x60 = gkeys.in(guest.product(key)).always();/*TEMPORAL OP*/ // gkeys in Guest -> Key -> Time
        Formula x86 = current.in(room.product(key)).always();/*TEMPORAL OP*/ // current in Room -> Key -> Time
        return Formula.compose(FormulaOperator.AND, x22,x27, x60,x86);
    }

    public Formula tempFacts (){
        Variable r=Variable.unary("r");
        Formula rt1 = (r.join(lastkey)).lone(); // all t : Time, r : Room | lone r.lastkey.t
        Formula rt2 = (r.join(current)).one(); // all t : Time, r : Room | one r.current.t
        Formula rt3 = (r.join(current)).in(r.join(rkeys)); // all t : Time, r : Room | r.current.t in r.keys
        return (rt1.and(rt2).and(rt3)).forAll(r.oneOf(room)).always();/*TEMPORAL OP*/
    }
    ...
    private Formula noIntervenes() {
        ...
        return x394.forAll(g.oneOf(guest).and(r.oneOf(room)).and(k.oneOf(key))).always();/*TEMPORAL OP*/
    }
    ...
    private Formula noBadEntries() {
        ...
        Formula x566=x574.forAll((r.oneOf(room)).and(g.oneOf(guest)).and(k.oneOf(key))).always();/*TEMPORAL OP*/
    ...
    private Formula entry(Variable r, Variable k, Variable g) {
        ...
        Expression x595=((r.join(current)).join(key_next.closure())).intersection(r.join(rkeys)); // (r.current.t).next+ & r.keys
        ...
        Formula x609=(r.join(current.post())).eq(k);/*TEMPORAL OP*/ // r.current.t' = k

        Variable r1=Variable.unary("r'");
        Formula x641=(r1.join(current)).eq(r1.join(current.post()));/*TEMPORAL OP*/
        Formula x637=x641.forAll(r1.oneOf(room.difference(r))); // all x : Room - r | x.current.t = x.current.t'

        Formula x614=gkeys.eq(gkeys.post());/*TEMPORAL OP*/ // gkeys.t = gkeys.t'
        Formula x629=(lastkey).eq(lastkey.post());/*TEMPORAL OP*/ // lastley.t = lastkey.t'
        Formula x647=(occupant).eq(occupant.post()); /*TEMPORAL OP*/ // occupant.t = occupant.t'
    ...
    private Formula checkin( Variable r, Variable k, Variable g) {
        Formula x398=g.join(gkeys.post()).eq((g.join(gkeys)).union(k)); // g.keys.t' = g.keys.t + k
        Formula x407=(r.join(occupant)).no(); // no r.occupant.t
        Formula x411=(lastkey.post()).eq((lastkey).override(r.product(k))); // lastkey.t' = lastkey.t ++ r -> k
        Expression x428=(((r.join(lastkey)).join(key_next.closure())).intersection(r.join(rkeys))).join(key_next.closure());
        ...
        Formula x439=(occupant.post()).eq((occupant).union(r.product(g))); // occupant.t' = occupant.t + r -> g
    ...
    private Formula checkout(Variable g) {
        ...
        Formula x343=occupant.post().eq((occupant).difference(room.product(g))); // occupant.t' = occupant.t - room -> g
