 ...
 * @author Emina Torlak 
 * @author Nuno Macedo // [pardinus] model finding hierarchy, decomposed model finding
	...
	public final static class SolutionIterator implements Iterator<Solution> {
		...
		private Solution nextNonTrivialSolution() {
			...
			if (opt.targetoriented()) {
				...
				// [pardinus] add the targets to generate the following solution
			...
			opt.reporter().solvingCNF(0, transl.numPrimaryVariables(), cnf.numberOfVariables(), cnf.numberOfClauses());
		...
		private Solution nextTrivialSolution() {
			...
			if (sol.instance()==null) {
				...
				// [pardinus] proper decomposed clone
				...
				// [pardinus] when integrated consider the amalgamated problem to generate the formula,
		...
		private class DefaultRetargeter implements Retargeter {
			...
			public void retarget(Translation transl) {
				...
				tcnf.clearTargets();
				// [pardinus] if there are weights must iterate through the relations to find the literal's owner
				if (weights != null) {
					...
					for (Relation r : transl.bounds().relations()) {
						...
						if (r.name().equals("Int/next") || r.name().equals("seq/Int") || r.name().equals("String")) {}
							...
							while (is.hasNext()) {
								int i = is.next();
								// [pardinus] add current model as weighted target
				...
				// [pardinus] if there are no weights may simply iterate literals
				else {
