package kodkod.test.pardinus.temporal;
        ...
        import kodkod.ast.*;
import kodkod.engine.ltl2fol.SlicingDynamicFormulas;
...
public class SlicingUnitTesting {
    ...
    public final void test1() {
        ...
        Formula initial2 = toSend.join(var3).eq(toSend.join(var3)).release(Process.join(toSend.post()).lone()).forAll(var3.oneOf(Process)).next();
    ...
    public final void test5() {
        Formula var1 = succ1.in(Process.product(Process)).implies(succ2.in(Process.product(Process)).and(succ.in(Process.product(Process))));
        Variable var3 = Variable.unary("p");
        Formula initial = toSend.join(var3).eq(toSend.join(var3)).until(Process.join(toSend.post()).lone()).forAll(var3.oneOf(Process)).next();
